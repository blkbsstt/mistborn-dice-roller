...
["", "mistborndiceroller.dart", , Y, {
  "^": "",
  main: [function() {
    P.print(J.toInt$0$n(J.get$valueAsNumber$x(H.interceptedTypeCast(document.querySelector("#poolSize"), "$isNumberInputElement"))));
    var t1 = J.get$onClick$x(document.querySelector("#roll"));
    H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(Y.rollDie$closure()), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)])._tryResume$0();
  }, "call$0", "main$closure", 0, 0, 1],
  rollDie: [function(e) {
    var t1, pool, result, result_comp, images, i, text;
    H.interceptedTypeCast(J.get$target$x(e), "$isButtonElement");
    t1 = J.toInt$0$n(J.get$valueAsNumber$x(H.interceptedTypeCast(document.querySelector("#poolSize"), "$isNumberInputElement")));
    pool = new Y.MistbornDicePool(t1 - P.min(10, t1), [], []);
    pool.DicePool$2$sides(P.min(10, t1), 6);
    pool.roll$0();
    P.print(pool.last_roll);
    result = pool.result$0(0);
    P.print(result.high_value);
    P.print(result.nudges);
    result_comp = document.querySelector("#dice_result");
    J._clearChildren$0$x(result_comp);
    images = [];
    for (t1 = pool.last_roll, t1 = new H.ListIterator(t1, t1.length, 0, null); t1.moveNext$0();) {
      i = t1._current;
      e = document.createElement("img", null);
      J.set$src$x(e, $.get$D6_imageMap().$index(0, i));
      images.push(e);
    }
    for (t1 = new H.ListIterator(images, images.length, 0, null); t1.moveNext$0();)
      result_comp.appendChild(t1._current);
    text = document.querySelector("#result_text");
    J._clearChildren$0$x(text);
    t1 = document.createElement("p", null);
    t1.textContent = "You rolled " + H.S(result) + ".";
    text.appendChild(t1);
  }, "call$1", "rollDie$closure", 2, 0, 10],
  count: function(it) {
    return H.IterableMixinWorkaround_fold(it, P.LinkedHashMap_LinkedHashMap(null, null, null, null, P.$int), new Y.count_closure());
  },
  Die: {
    "^": "Object;sides",
    roll$0: function() {
  ...
  DicePool: {
    "^": "Object;",
    roll$0: function() {
      var t1 = H.setRuntimeTypeInfo(new H.MappedListIterable(this.dice, new Y.DicePool_roll_closure()), [null, null]).toList$0(0);
      this.last_roll = t1;
      return t1;
    },
  ...
  MistbornDicePool: {
    ...
    result$0: function(_) {
      var freq = Y.count(this.last_roll);
      freq.putIfAbsent$2(6, new Y.MistbornDicePool_result_closure());
      return new Y.RollResult(H.setRuntimeTypeInfo(new H.WhereIterable(this.last_roll, new Y.MistbornDicePool_result_closure0(freq)), [null]).fold$2(0, 0, P.max$closure()), J.$add$ns(freq.$index(0, 6), this.nudges));
    }
  },
  ...
  count_closure: {
    ...
    call$2: function(map, i) {
      map.putIfAbsent$2(i, new Y.count__closure());
      map.$indexSet(0, i, J.$add$ns(map.$index(0, i), 1));
      return map;
    }
  },
  count__closure: {
  ...
  Range: {
    ...
    $eq: function(_, other) {
      ...
      if (t1 == null ? t3 == null : t1 === t3) {
        t1 = this.stop;
        t2 = t2.get$stop(other);
        t1 = (t1 == null ? t2 == null : t1 === t2) && this.step === other.step;
      } else
        t1 = false;
      return t1;
    },
...
Isolate.$lazy($, "imageMap", "D6_imageMap", "get$D6_imageMap", function() {
  return P.LinkedHashMap_LinkedHashMap$_literal([1, "http://upload.wikimedia.org/wikipedia/commons/c/c5/Dice-1.png", 2, "http://upload.wikimedia.org/wikipedia/commons/1/18/Dice-2.png", 3, "http://upload.wikimedia.org/wikipedia/commons/7/70/Dice-3.png", 4, "http://upload.wikimedia.org/wikipedia/commons/a/a9/Dice-4.png", 5, "http://upload.wikimedia.org/wikipedia/commons/6/6c/Dice-5.png", 6, "http://upload.wikimedia.org/wikipedia/commons/5/5c/Dice-6.png"], null, null);
});
// Native classes
...
function dart_precompiled($collectedClasses) {
  ...
  if (!"name" in Die)
    Die.name = "Die";
  $desc = $collectedClasses.Die;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Die.prototype = $desc;
  function DicePool() {
  }
  DicePool.builtin$cls = "DicePool";
  if (!"name" in DicePool)
    DicePool.name = "DicePool";
  $desc = $collectedClasses.DicePool;
  if ($desc instanceof Array)
    $desc = $desc[1];
  DicePool.prototype = $desc;
  function DicePool_roll_closure() {
  }
  DicePool_roll_closure.builtin$cls = "DicePool_roll_closure";
  if (!"name" in DicePool_roll_closure)
    DicePool_roll_closure.name = "DicePool_roll_closure";
  $desc = $collectedClasses.DicePool_roll_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  DicePool_roll_closure.prototype = $desc;
  function MistbornDicePool(nudges, dice, last_roll) {
    this.nudges = nudges;
    this.dice = dice;
    this.last_roll = last_roll;
  }
  MistbornDicePool.builtin$cls = "MistbornDicePool";
  if (!"name" in MistbornDicePool)
    MistbornDicePool.name = "MistbornDicePool";
  $desc = $collectedClasses.MistbornDicePool;
  if ($desc instanceof Array)
    $desc = $desc[1];
  MistbornDicePool.prototype = $desc;
  function MistbornDicePool_result_closure() {
  }
  MistbornDicePool_result_closure.builtin$cls = "MistbornDicePool_result_closure";
  if (!"name" in MistbornDicePool_result_closure)
    MistbornDicePool_result_closure.name = "MistbornDicePool_result_closure";
  $desc = $collectedClasses.MistbornDicePool_result_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  MistbornDicePool_result_closure.prototype = $desc;
  function MistbornDicePool_result_closure0(freq_0) {
    this.freq_0 = freq_0;
  }
  MistbornDicePool_result_closure0.builtin$cls = "MistbornDicePool_result_closure0";
  if (!"name" in MistbornDicePool_result_closure0)
    MistbornDicePool_result_closure0.name = "MistbornDicePool_result_closure0";
  $desc = $collectedClasses.MistbornDicePool_result_closure0;
  if ($desc instanceof Array)
    $desc = $desc[1];
  MistbornDicePool_result_closure0.prototype = $desc;
  function RollResult(high_value, nudges) {
    this.high_value = high_value;
    this.nudges = nudges;
  }
  RollResult.builtin$cls = "RollResult";
  if (!"name" in RollResult)
    RollResult.name = "RollResult";
  $desc = $collectedClasses.RollResult;
  if ($desc instanceof Array)
    $desc = $desc[1];
  RollResult.prototype = $desc;
  function count_closure() {
  }
  count_closure.builtin$cls = "count_closure";
  if (!"name" in count_closure)
    count_closure.name = "count_closure";
  $desc = $collectedClasses.count_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  count_closure.prototype = $desc;
  function count__closure() {
  }
  count__closure.builtin$cls = "count__closure";
  if (!"name" in count__closure)
    count__closure.name = "count__closure";
  $desc = $collectedClasses.count__closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  count__closure.prototype = $desc;
